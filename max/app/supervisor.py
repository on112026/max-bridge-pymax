"""Главный цикл supervisor'а max-процесса (этап 2, PyMax).
...
- При ошибке Client (особенно ``error.limit.violate``) — возвращаемся в
  ``auth_required`` с ``notify_message``, не зацикливаемся.
...
Структура пакета ``max/app/supervisor/``:

* ``_backoff``     — backoff-параметры и хелперы.
* ``client_runtime`` — ``build_client`` + ``_long_running_start``.
* ``cache``        — ``wipe_cache`` + ``find_session_files`` + ``watch_session_file``.
* ``twofa_drain``  — ``drain_2fa_codes_loop``.
* ``read_receipts``— ``read_receipts_loop``.

Этот файл — тонкая публичная обёртка: ``run()`` собирает state-machine
обработки команд и lifecycle Client. ``max/run.py`` импортирует
``from app.supervisor import run``.
"""
